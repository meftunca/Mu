...
class console
{
    ...
    {
        return $this->getColoredString('// ========== [+] Mu php Web Framework Console Komutlari ========== //', 'red') . "\n\n" .
            ...
            $this->getColoredString('[-v]', 'light_blue') . "\t\t\t" . $this->getColoredString('Mu php Web Framework versiyon bilgisini verir.') . "\n" .
            $this->getColoredString('[-h]', 'light_blue') . "\t\t\t" . $this->getColoredString('Tum console komutlari ile ilgili bilgi verir.') . "\n\n" .
            $this->getColoredString('// ========== [-] Mu php Web Framework Console Komutlari ========== //', 'red');
    }
    ...
    {
        return $this->getColoredString('Mu php Web Framework: ', 'light_red') . "\t" . $this->getColoredString('v' . VERSION, 'light_blue');
    }
    ...
    private function createController($controller)
    {
        ...
        if ($isDir) {
            ...
            $class      = end($parts);
            array_pop($parts);
            $namespace  = 'app\\controller\\' . implode('\\', $parts);
        } else {
            $namespace  = 'app\\controller';
            $class      = $controller;
        }

        $location   = 'app/controller/' . $controller . '.php';
    ...
    private function createmodel($model)
    {
        ...
        if ($isDir) {
            ...
            $class      = end($parts);
            array_pop($parts);
            $namespace  = 'app\\model\\' . implode('\\', $parts);
        } else {
            $namespace  = 'app\\model';
            $class      = $model;
        }
    ...
    private function createListener($listener)
    {
        ...
        $location   = 'app/listener/' . $listener . '.php';
    ...
    {
        array_map('unlink', glob("app/storage/Log/*"));
