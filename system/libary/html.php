...
//basics
    ...
    public static function style_array($value = [])
    {

        foreach ($value as $key) {
            echo '<link rel="stylesheet" href="' . css . $key . '.css">';
        }

    }
    //css dosya include etme yöntemi
    // css('style') yada çok dosya ise dizi olarak css(array('style','bootstrap'....))
    public static function script_array($value = [])
    {
        foreach ($value as $key) {
            echo '<script type="text/javascript" src="' . js . $key . '.js"></script>';
            # code...
        }
     ...
     * @return string
     */
    public static function openGraph($url, $type = null, $title = null, $descr = null, $image = null)
    {
        $response = '';
        if (is_array($url)) {
     ...
     * @return string
     */
    public static function twitterCards($card, $url = null, $title = null, $descr = null, $image = null)
    {
        $response = '';
        if (is_array($card)) {
     ...
     * @return string
     */
    public static function image($url, $alt = null, $attr = [])
    {
        $image = '<img src="' . $url . '" ';
        if (!is_null($alt))
            $image .= 'alt="' . $alt . '" ';
     ...
     * @return string
     */
    public static function link($url, $title, $attr = [])
    {
        $link = '<a href="' . $url . '" ';
        if (!empty($attr)) {
            foreach ($attr as $key => $val) {
